...
# ---------------------------------------------------------------------------
# Fix H (2026-05-17) — Grounding gate admits category aggregates and loose
# rounding for analyst-mode aggregation queries. Triggered by Q2 production
# trace 5fb94f75 where the LLM produced cross-column "thermal average ≈ 188"
# from pairs (price_regulated_old_tpp_gel, price_regulated_new_tpp_gel) and
# loose-rounded 15.41 → 15.5; both were dropped by strict_numeric 0.9.
# ---------------------------------------------------------------------------


def _make_analyst_aggregation_ctx(df_payload: dict, query: str = "Average X by category"):
    """Helper: build a QueryContext in analyst mode with needs_average set."""
    import pandas as pd
    from models import QueryContext
    from contracts.question_analysis import (
        AnalysisMode, AnswerKind, ClassificationInfo, Grouping,
        KnowledgeInfo, LanguageCode, LanguageInfo, PreferredPath, QueryType,
        QuestionAnalysis, RenderStyle, RoutingInfo, SqlHints, ToolingInfo,
        AnalysisRequirementsInfo, VisualizationInfo,
    )
    ...
    qa = QuestionAnalysis(
        ...
        tooling=ToolingInfo(candidate_tools=[]),
        sql_hints=SqlHints(),
...
def test_h2_grounding_admits_pairwise_column_group_average():
    """Fix H.2 — cross-column category average like thermal ≈ mean(new_tpp, old_tpp)
    must register as a grounded token, not a hallucination.

    With per-column means of 195.0 and 181.0, the pair-wise mean is 188.0.
    The LLM's category-aggregate citation '188' must pass the gate.
    """
    from agent import summarizer
    from core.llm import SummaryEnvelope

    ctx = _make_analyst_aggregation_ctx({
        "date": ["2024-01-01", "2024-02-01", "2024-03-01"],
        "price_regulated_old_tpp_gel": [200.0, 195.0, 190.0],
        "price_regulated_new_tpp_gel": [185.0, 181.0, 177.0],
    }, query="Compare thermal average prices in 2024")

    # Answer cites the cross-column category mean (188 = (195+181)/2).
    envelope = SummaryEnvelope(
        answer="The thermal sellers had an average price near 188 in 2024.",
        claims=[],
        citations=["data_preview"],
        confidence=0.9,
    )
...
def test_h2_grounding_pair_requires_matching_unit_suffix():
    """Fix H.2 — pair-grouping must not mix units. ``..._tpp_gel`` and
    ``..._tpp_usd`` differ in the last token (unit), so they should NOT
    form a pair group. A mean of (mean_gel, mean_usd) would be nonsensical."""
    from agent import summarizer

    # Direct test of the helper: confirm only same-unit siblings group.
    pairs = summarizer._iter_pairwise_column_groups([
        "price_regulated_old_tpp_gel",
        "price_regulated_new_tpp_gel",
        "price_regulated_old_tpp_usd",
        "price_regulated_new_tpp_usd",
    ])
    # Expect exactly two pairs: (old_gel, new_gel) and (old_usd, new_usd)
    assert ("price_regulated_old_tpp_gel", "price_regulated_new_tpp_gel") in pairs
    assert ("price_regulated_old_tpp_usd", "price_regulated_new_tpp_usd") in pairs
    # And NOT mixed-unit pairs
    assert ("price_regulated_old_tpp_gel", "price_regulated_old_tpp_usd") not in pairs
    assert ("price_regulated_new_tpp_gel", "price_regulated_new_tpp_usd") not in pairs


def test_h1_grounding_admits_loose_rounding_of_column_mean():
    """Fix H.1 — LLMs round aggregates to human-readable steps (nearest 0.5,
    1, 5, 10). A column mean of 15.41 cited as '15.5' must register as
    grounded, even though it doesn't fall on nearest-0.1 rounding.
    """
    from agent import summarizer
    from core.llm import SummaryEnvelope
    ...
    ctx = _make_analyst_aggregation_ctx({
        "date": ["2024-01-01", "2024-02-01", "2024-03-01"],
        # mean = 15.41 — loose-rounds to 15.5 (nearest 0.5)
        "price_regulated_hpp_gel": [15.10, 15.41, 15.72],
    }, query="Average HPP price 2024")

    envelope = SummaryEnvelope(
        answer="Small hydro had an average price near 15.5 in 2024.",
        claims=[],
        citations=["data_preview"],
        confidence=0.9,
    )
...
def test_h3_grounding_relaxes_threshold_for_aggregation_analyst_query():
    """Fix H.3 — analyst-mode + aggregation_intent.needs_average should
    relax STRICT_NUMERIC from 0.9 to 0.75, letting answers with one or two
    derived aggregates pass even when not every value tokenizes.

    Setup: 4 tokens in the answer, only 3 match the source (ratio 0.75).
    At 0.9 this would fail; at 0.75 (Fix H.3) it should pass.
    """
    from agent import summarizer
    from core.llm import SummaryEnvelope

    # Single isolated column so H.1/H.2 don't produce 15.5 organically;
    # this test isolates H.3 (threshold lowering) from H.1/H.2 (token
    # expansion). Mean of [50.0, 51.0] = 50.5 → tokens cover 50/51/50.5
    # but NOT 99.9.
    ctx = _make_analyst_aggregation_ctx({
        "date": ["2024-01-01", "2024-02-01"],
        "isolated_metric_gel": [50.0, 51.0],
    }, query="Average price 2024")

    # 4 tokens: 50, 51, 50.5 — all groundable; plus 99.9 — ungroundable.
    # Without H.3, ratio = 3/4 = 0.75, which would FAIL at 0.9 threshold.
    envelope = SummaryEnvelope(
        answer="Values include 50, 51, and 50.5, plus a derived 99.9.",
        claims=[],
        citations=["data_preview"],
        confidence=0.8,
    )
...
def test_h3_strict_numeric_threshold_unchanged_without_aggregation_intent():
    """Negative control: when aggregation_intent is empty (no
    needs_average/total/share), STRICT_NUMERIC stays at 0.9. A 0.75-ratio
    answer must still fail. Guards against the H.3 relaxation leaking
    into non-aggregation queries.
    """
    from agent import summarizer
    from core.llm import SummaryEnvelope
    from models import QueryContext
    ...
    ctx = QueryContext(
        query="Show price for January 2024",
        preview="date,p_bal_gel\n2024-01-01,50.0\n2024-02-01,51.0",
        stats_hint="",
        grounding_policy="strict_numeric",
        # No aggregation_intent set — default {} empty dict.
        mode="analyst",
    )
    envelope = SummaryEnvelope(
        answer="Values include 50, 51, and 50.5, plus a derived 99.9.",
    ...
    # 3/4 = 0.75, below the 0.9 STRICT_NUMERIC threshold, must fail.
    assert summarizer._is_summary_grounded(envelope, ctx) is False


def test_h3_strict_numeric_threshold_unchanged_for_non_analyst_aggregation():
    """Negative control: aggregation_intent is set, but mode is NOT analyst.
    The H.3 relaxation must require BOTH signals to fire; non-analyst
    aggregation queries keep the 0.9 threshold.
    """
    from agent import summarizer
    from core.llm import SummaryEnvelope
    from models import QueryContext
    ...
    ctx = QueryContext(
        ...
        mode="conversational",  # NOT analyst
    )
    envelope = SummaryEnvelope(
        answer="Values include 50, 51, and 50.5, plus a derived 99.9.",
        claims=[],
