...
**B3.B / B4.B authenticated smoke — GREEN (2026-07-19).** Frozen frontend identity of record: **`193896745b5fd985da6618c366e750b73887dd75`**. All three surfaces curl-verified aligned at that SHA: browser `release-manifest.json` `app_version`, Edge `X-Enai-Edge-Version`, and the Edge healthcheck version; backend `/healthz`+`/readyz` 200. **Post Deploy Smoke #240 passed** with `SMOKE_REQUIRE_AUTH=true` — the authenticated browser→Edge→backend path (frontend root + security headers, deployed-artifact verification, login/public routes, healthcheck, Supabase sign-in + `/is-admin`). Frontend CI #272 and Edge deploy #18 green at the same SHA.

**Artifact-integrity model decision (option B, recorded).** Post Deploy Smoke originally gated on the deployed bundle aggregate equalling a freshly-built CI aggregate. Railway **rebuilds the frontend from source**, so the deployed bytes are not byte-identical to a separate CI build of the same commit (esbuild/toolchain nondeterminism across environments; `.nvmrc` and `caniuse-lite` are pinned, so Node/browserslist are not the cause). The deployed app is nonetheless the correct source: `app_version` = the exact SHA, and every served file matches the deployed manifest. Frontend commit `1938967` realigns the gate with this deployment model — identity (`app_version` == expected SHA) + manifest self-consistency + per-file served-bytes integrity are blocking; cross-build byte equality is a non-blocking diagnostic warning. This mirrors the plan's accepted backend posture: *"Railway rebuilt from Git… the SHA comparison proves source identity but the… digest will differ; do not claim byte-identical promotion."* **Honest scope:** this proves "a self-consistent build of the exact source SHA is deployed and served intact," not byte-identical artifact promotion; the stronger guarantee would require build-once/promote-by-artifact (deferred, not adopted).

Remaining for B4.B closure: **Live Browser Proof** (Playwright authenticated dashboard/admin) + credentialed **axe** (`test:e2e:a11y`), the paused/quota-exhausted synthetic-account denial states, and the production-safe read-only DB checks.
