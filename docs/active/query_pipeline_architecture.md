...
Active cross-check (`_cross_check_answer_kind`) compares the LLM-emitted `answer_kind` against a `query_type`-derived value every call. On disagreement the safer option is preferred unless the legal-list exception fires (LIST + regulatory/conceptual + confidence ≥ 0.85). The scenario-metric override (EXPLANATION/SCALAR/TIMESERIES → SCENARIO when the analyzer emits a scenario-family derived metric) is gated by the shared deterministic scenario contract: the numerical parameter must match the raw question and must be bound to the same subject the request transforms. Bare dates, mismatched factors, and cross-metric causal counterfactuals do not qualify.

**Guardrail reading rule (2026-08-09).** The deterministic guardrails in [`agent/question_interpretation.py`](../../agent/question_interpretation.py) that coerce analyzer output onto a fixed route split their inputs in two. **Positive conditions** — what the question is about, what it asks for, and the metric or pair the guardrail routes to — read `_asked_question_surface`, the question alone. **Negative brakes** read the whole text, because a disqualifying mention anywhere is still a reason to leave the analyzer's routing alone. A report track query carries its research title, coverage bullets, and the entire report request beneath the question; matching a positive condition inside those lines classifies a track by its neighbours. That has cost four separate misroutes (jobs 37567e5f, 40e55527, and the May-2026 supply track that a context-borrowed "evolution" turned into a full-history trend). Add no positive condition that reads past `_asked_question_surface`.
