...
| `ENAI_AUTH_MODE=gateway_and_bearer` / dormant `ENABLE_PUBLIC_BEARER_AUTH` surface | **B6 decision (2026-07-20, Irakli): KEEP as a guarded, default-off protocol option — classified *protocol/data compatibility*, not expired.** Owner: Irakli. Production runs `gateway_only`; the surface is unreachable unless `ENAI_AUTH_MODE=gateway_and_bearer` is deliberately set, and its JWT verification policy is pinned by `tests/test_auth_negative.py` (alg/claim/expiry/audience matrix). **Removal criterion:** delete the mode, `ENABLE_PUBLIC_BEARER_AUTH`, `_verify_supabase_token`, the bearer branch in `authenticate_request`, the `main.py` public-bearer rate-limit branch, and the now-unused PyJWT dependency **once a product decision confirms public bearer auth will never ship**. Removal is a self-contained auth-core change deliberately deferred out of the B6 closure window to avoid refactoring the production auth path during certification. | Removal reviewed by 2026-09-30 |
