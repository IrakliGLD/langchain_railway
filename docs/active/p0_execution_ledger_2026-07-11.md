...
**Date:** 2026-07-11
**Status:** P0 code is locally verified; repository commits are independent and production activation remains a manual follow-up
...
| P0.2 truthful release gates (M24) | Verified locally | The active export tree passes clean `npm ci`, lint, 228 fail-on-console tests, build, and production audit. A canonical-repository port passes lint, all 318 tests, build, and the production audit using the identical clean-installed lock graph. CI cannot skip DB tests; the negative console fixture exits nonzero. | Install/commit the audited port in the frontend Git repository, obtain pinned Node 20/Python 3.11 CI results, then run DB, post-deploy/browser smoke jobs and verify branch protection. These do not hold the backend commit. QA/release lead. |
...
| P0.9 null averages (H4) | Verified locally | The active frontend and canonical-repository port use an unweighted finite-observation denominator for price/tariff; null, zero, all-missing, order, and tariff regressions pass | Install/commit the canonical port, deploy it, and confirm production telemetry. Frontend lead. |
| P0.10 readiness/startup (M17) | Verified locally | Readiness requires DB plus every expected relation/column, refreshes schema state for recovery/drift detection, and reflection cannot expand the SQL allow-list. A real subprocess test proves forced Uvicorn startup failure exits nonzero. | Verify platform probe/restart behavior and readiness-query cost after deployment. Platform/SRE lead. |
...
- Backend startup remains one Uvicorn worker in source. Actual Railway replica count is not locally observable.
...
- A path-by-path P0 port based on the tracked canonical frontend state is verified at `D:/Enaiapp/p0_frontend_repo_merge`. It preserves canonical-only UI coverage and the chat-history role tie-breaker. It still needs installation in a dedicated linked worktree of the frontend Git repository before its independent commit.
- The frontend repository's current worktree remains on `docs/progressive-thinking-text-spec` with unrelated untracked logs, build output, local settings, and a plan. P0 must use explicit paths on a dedicated branch; never use `git add -A` there.
- Deployed edge hashes, live RLS/grants, provider timeouts, least-privilege runtime identity, branch protection, and production access logs remain manual verification items; lack of access does not block either code commit.
...
| Medium | Startup-only schema reflection could stay degraded after database recovery or over-report readiness after runtime schema drift | Fixed: `/readyz` refreshes schema state whenever connectivity succeeds; recovery and drift regressions pass. |
...
| High | Copying the active export snapshot over the canonical frontend repository would regress the chat v3.3 equal-timestamp role tie-breaker and remove canonical-only UI tests | Avoided with a path-by-path port: chat is v3.4 with both protections, and Builder/Chat/Dashboard retain 30/13/17 tests. Install this exact audited port in a dedicated frontend worktree rather than copying the outer tree wholesale. |
