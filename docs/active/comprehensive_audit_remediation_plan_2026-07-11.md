...
**F4 P5.1 deadline completion (2026-07-16):** Backend commit `85f3b64` propagates the actor-bound request deadline into DB, chat-model, embedding, coalescing, and secondary-evidence boundaries; applies request-local PostgreSQL statement timeouts after read-only transaction setup; disables SDK retry amplification; gives OpenAI, Gemini, and NVIDIA independent bounded timeouts; permits only one budget-checked jittered fallback after proven pre-delivery rejection; and records content-free provider attempts by hashed actor/request identity. Independent frontend/Supabase commit `47a437d` makes the durable chat-operation ledger explicitly distinguish pre-backend from ambiguous post-backend failure. Backend Ruff and all `1,591` tests pass; pinned Deno 2.1.4 format/lint/type-check and all `21` Edge tests pass. Local implementation is complete. Production remains open for backend/Edge deployment plus the live latency, billing/execution-count, durable-ledger, DB-timeout, provider-timeout, and one-replica evidence in [f4_deadline_failure_matrix_2026-07-16.md](f4_deadline_failure_matrix_2026-07-16.md). No SQL patch or browser deployment is required.
**F6 P5.6 session-containment update (2026-07-17):** Backend commit `4fc5b2d` completes the supported one-process/one-replica P5.6 containment path. It preserves P3 actor/session assertions, binds session tokens/history/contracts to actor plus authentication mode, serializes complete actor-bound turns with request-budget-bounded waiting, rejects live cross-owner replay, prevents expired/evicted records from being resurrected by old tokens or mutations, bounds count/TTL/history bytes with inactive eviction, and exposes content-free protected metrics. The existing worker guard still rejects configured worker counts other than one. All `1,623` backend tests and full Ruff pass. No frontend, Edge Function, or SQL change is required. Deployment/smoke and Railway evidence proving exactly one replica with autoscaling disabled remain manual; shared state is a future conditional capability and is required only before horizontal scaling. See [f6_session_ownership_and_scaling_containment_runbook_2026-07-17.md](f6_session_ownership_and_scaling_containment_runbook_2026-07-17.md).
**F7 P6 frontend activation evidence (2026-07-17):** Independent frontend commit `a4e9a7e` closes the remaining locally reproducible P6 responsive-accessibility gaps with a compact 46-pixel mobile rail, accessible overlay drawer/focus restoration, non-collapsing segment controls, and real public-route viewport regression coverage. Frontend lint, all `456` tests, production build/artifact verification, database patch verifiers, release-contract checks, browser measurements at 390/768/1024 pixels, mobile Escape/focus behavior, and a clean deployed-public console passed. P6 remains production-open for the live JSONB migration/constraints, credentialed admin/browser smoke, credentialed axe, assistive-technology/keyboard/touch review, and true 200 percent zoom evidence recorded in the frontend F7 runbook. No backend file or deployment dependency was introduced.

**F8 P7 privacy and production assurance (2026-07-17):** Backend commit `9822cff` adds a content-free synthetic log-canary scanner, a read-only inherited/PUBLIC privilege inventory, a cross-service retention/access/export/deletion/incident and rollback runbook, and moves `pytest` out of production dependencies. Independent frontend commit `8f4898b` pins the release-evidence checkout/setup-node/upload actions to immutable official commits. Local evidence is `19` focused backend tests and Ruff passing, a clean scanner self-check, a CycloneDX 1.6 backend SBOM with `22` direct components, `6` frontend release-contract tests, a zero-vulnerability frontend production audit, and a CycloneDX 1.5 frontend SBOM with `121` components. The backend full advisory scan is still Unverified locally because this workstation only has Python 3.14 while the pinned pandas/numpy runtime targets Python 3.11; Docker is also unavailable. Run the protected Python 3.11 exact-SHA workflow. P7 remains production-open for privacy-owner/vendor-console approval, deployed log canaries, legacy archive disposition, PUBLIC/network/credential maintenance, exact deployed-artifact identity, one-replica/autoscaling evidence, smoke, and rollback rehearsal. See [f8_p7_privacy_production_assurance_runbook_2026-07-17.md](f8_p7_privacy_production_assurance_runbook_2026-07-17.md).
**F9 P8.A incremental structural completion (2026-07-18):** The requested behavior-neutral backend extraction sequence is complete in separate commits: startup/readiness `b291018`, actor-bound session runtime `a9cfb57`, provider invocation `65bcb10`, deadline-aware stage orchestration `459f555`, typed evidence-plan validation `483b4ce` plus compatibility audit fix `48b65f6`, question-contract finalization `8c36ed8`, provenance-safe evidence derivation `730c0c6`, and summary grounding/provenance gates `5ef774c`. Characterization and boundary tests protect the stable interfaces; public schemas, tool/model-call behavior, golden rendering, provenance, and deadline semantics were not intentionally changed. Final local evidence is all `1,669` backend tests passing plus clean Ruff and diff checks. This completes the F9 structural batch, not all of P8: Stage 0.7 strategy deletion, LIGHT-tier retrieval changes, and rollout-flag retirement remain prohibited until their production evidence gates pass. The separate P8.B Vite/esbuild upgrade remains deferred.
...
| P5.A/P5.B | P5.1 local implementation is complete in backend `85f3b64` and independent frontend/Supabase `47a437d`; the operator reported F4 deployed on 2026-07-16. P5.3 global DB coordination is locally implemented in backend `ec5db1e`, building on DB gateway `ba8dd11`. P5.6 one-replica session containment is locally complete in backend `4fc5b2d`. | Record the F4 live deadline/provider/ledger evidence; deploy F5, measure Supabase/Supavisor capacity, and record saturation/readiness/cancellation metrics. Deploy/smoke F6 and retain Railway proof of exactly one replica with autoscaling disabled. Shared state remains a future prerequisite only if horizontal scaling is approved. |
| P6.A/P6.B | P6.1 is locally complete across the strict backend/frontend generated contract. P6.2-P6.7 implementation is complete, and F7 responsive/accessibility closure is committed independently in frontend `a4e9a7e` with 456 tests, lint/build/artifact/patch checks, viewport/focus evidence, and a clean public console. | Apply/finish the P6.2 JSONB migration and constraints; deploy compatible contract consumers; run credentialed bounded-admin and state smoke; complete credentialed axe, assistive-technology/keyboard/touch, and true 200 percent zoom evidence. |
| P7.A/P7.B | Backend F8 implementation is committed in `9822cff`; frontend F8 release-workflow hardening is independently committed in `8f4898b`, building on P7 packages `4e7d0bb`/`5fa8be1` and one-replica containment `4fc5b2d`. | Production gate remains open: privacy-owner/vendor-console approval, legacy archive disposition, deployed log canary, PUBLIC/network/obsolete-credential maintenance, exact-SHA workflow/artifact identity, exactly-one-replica/autoscaling proof, smoke, and rollback rehearsal. |
| P8.A/P8.B | P8.A's requested F9 structural sequence is complete through independently audited commits from `b291018` to `5ef774c`, with all `1,669` backend tests passing. P8.B has the prior generated-artifact/toast debt reductions. | P8.A remains production-evidence-open for Stage 0.7 counters, LIGHT cost/latency/relevance/grounding evidence, and two-stable-release flag cleanup. Further risk-driven coverage is ongoing debt rather than an F9 exit blocker. The separate P8.B Vite/esbuild major upgrade remains open. |
...
- [x] Decide whether deleted users remain as UUIDs or become stable pseudonymous identifiers. Decision: retain UUIDs only in the protected append-only audit trail for 24 monthly buckets; operational logs use keyed hashes.
...
**Track status (2026-07-16):** P5.1 local implementation is complete across independent backend commit `85f3b64` and frontend/Supabase commit `47a437d`, building on `9bbe444`, `a199c88`, and `a61de51`. The repositories remain independently buildable and deployable and share only HTTP identity/deadline/error contracts. Production acceptance remains open until the immutable backend and Edge artifacts are deployed and the live failure matrix proves deadline tolerance, provider execution/billing counts, durable ambiguous-delivery state, and one-replica topology.
...
- [x] Propagate remaining time into DB and model-provider calls and ensure already-running I/O cooperatively stops where possible.
...
- [x] Add explicit DB/provider timeouts, jitter policy, and conservative ambiguous-delivery reconciliation; portable provider cancellation/status lookup is unavailable, so ambiguous delivery is never blindly replayed.
...
**Additional backend P5.1 slice (2026-07-15):** Commit `d9ec12b` (H13) added the initial explicit OpenAI request-timeout slice. The remaining local implementation work identified here was subsequently completed by F4 backend commit `85f3b64`; only the production attestation described below remains open.

**F4 completion evidence (2026-07-16):** Backend commit `85f3b64` adds a request execution scope carrying the monotonic deadline, safe request ID, and HMAC-hashed actor identity into DB/model/embedding calls and copied secondary-worker contexts. PostgreSQL statement timeout is the smaller of the configured cap and remaining-minus-cleanup budget; read-only setup is applied first. OpenAI-compatible SDK retries are zero, Google attempts are one total call, all three chat providers and both embedding providers receive remaining-time-bounded native timeouts, retry jitter is bounded, and a fallback begins only after a proven pre-delivery rejection with sufficient budget. Content-free provider-attempt records block duplicate actor/request/provider/stage execution; timeouts/network/5xx are ambiguous and non-replayable, while the durable Edge ledger in independent frontend commit `47a437d` marks post-backend failures chargeable. Backend Ruff and all `1,591` tests pass; pinned Deno 2.1.4 format/lint/type-check and all `21` Edge tests pass with manifest `e0111671ca86dcc3a29c088290c60ec608ec4181d2ce2ba7f2bbe68b0b7ce85e`. The remaining unchecked proof item is a production attestation, not missing local code: deploy both immutable commits and record real provider billing/execution counts, total latency tolerance, durable operation state, and DB/provider failure injection under [f4_deadline_failure_matrix_2026-07-16.md](f4_deadline_failure_matrix_2026-07-16.md).
...
- [x] Replace per-request executors with a process-wide bounded coordinator/semaphore.
- [x] Reserve capacity for control/readiness work.
- [x] Include pool checkout/queue time in the request deadline.
- [x] Support cancellation of DB-side work where possible.
- [ ] Set concurrency from measured Supabase/Supavisor capacity, not local worker count; safe configurable defaults exist, but the hosted allocation and production selection remain operator evidence.

**Backend implementation evidence (2026-07-16):** Commit `ec5db1e` adds one process-wide bounded DB-work coordinator, separate application/control admission, a configurable SQLAlchemy pool ceiling, deadline-bounded queue admission, aggregate protected metrics, and a bounded global Stage 0.8 executor. Secondary work inherits an actor-bound request identity and child deadline; queued futures are cancelled at parent exit, PostgreSQL `statement_timeout` bounds running SQL, and any non-cooperative overrun is counted rather than hidden. Schema readiness and runtime-identity probes use the reserved control lane. The audit also corrected half-open breaker ownership so a request expiring after queue admission cannot report a false database success. Ruff and diff checks passed; a 369-test main/DB/pipeline/evidence/vector/security regression set passed, alongside focused capacity/config/metrics tests. Activation and capacity selection are documented in [f5_global_db_coordination_runbook_2026-07-16.md](f5_global_db_coordination_runbook_2026-07-16.md).

**Current P5.3 state:** Local implementation and synthetic failure/load coverage are complete. Production acceptance remains blocked only on measuring the hosted Supabase/Supavisor allocation, deploying `ec5db1e`, and recording protected `resilience.db_work` evidence for normal traffic, saturation, pool pressure, open breaker, readiness, cancellation, and simultaneous primary/secondary work. A generic Python thread cannot be force-killed safely; production acceptance therefore requires `secondary_outstanding = 0` and no increase in `secondary_orphaned` after PostgreSQL cancellation and the documented cleanup tolerance.
...
- [x] Audit and retain the P3 signed gateway actor/session assertion and actor-derived authoritative session identity rather than adding a parallel binding mechanism.
- [x] Bind session token, conversation history, and contract snapshot to actor and authentication mode.
- [x] Serialize complete turns per actor-bound session with a wait bounded by both configuration and the remaining request budget.
- [x] Reject live cross-actor or cross-auth-mode session-token replay before pipeline/provider execution.
- [x] Prevent valid tokens whose records expired, were evicted, or were lost on restart from recreating the old empty session identity; mutations cannot recreate missing sessions.
- [x] Add explicit idle expiry, hard entry/field/turn bounds, inactive eviction, active-turn pinning, and content-free protected metrics.
...
- [ ] Before any approved horizontal scaling, externalize session/continuity, rate-limit, assertion-replay, and idempotency state together and pass two-process failure tests. This is a conditional future capability, not required remediation while one replica is enforced.

**Backend implementation evidence (2026-07-17):** Commit `4fc5b2d` implements the supported one-replica containment path and adds the activation/monitoring procedure in [f6_session_ownership_and_scaling_containment_runbook_2026-07-17.md](f6_session_ownership_and_scaling_containment_runbook_2026-07-17.md). Full Ruff, Python compilation, and all `1,623` backend tests pass. No frontend, Edge Function, database migration, or cross-repository file dependency changed.

**Acceptance:**

- Local tests prove cross-owner replay fails while the owned session remains unchanged.
- Local multithreaded tests prove deterministic same-session ordering, bounded overlap failure, and active-turn eviction protection.
- Local tests prove expired records receive a distinct identity and cannot be resurrected by later mutations.
- Configuration/startup tests prove unsupported in-process worker counts fail closed.
- Production acceptance still requires deployment smoke plus retained Railway evidence of one replica and disabled autoscaling.
- Two-process shared-state acceptance becomes mandatory only before the topology is changed beyond one replica.

### P5 exit gate

**Current state (2026-07-17):** P5 remains partially complete. P5.1 local implementation is complete in backend `85f3b64` and frontend/Supabase `47a437d`, and the operator reported F4 deployed; detailed live latency/provider/billing/durable-ledger failure-injection evidence remains unrecorded. P5.2 (`ba8dd11`), P5.4/P5.5 (`2c9e12b`), and P5.3 local implementation (`ec5db1e`) are complete. P5.3 still needs hosted capacity selection, deployment, and live load/cancellation/readiness evidence. P5.6 local one-replica containment is complete in `4fc5b2d`; its backend deployment/smoke and Railway replica/autoscaling attestation remain open. External shared state and two-process tests are deferred unless horizontal scaling is approved. Keep one backend replica.
...
- One-replica control-plane attestation remains mandatory; external shared-state integration becomes an exit gate only before horizontal scaling.
...
#### P7.A backend subtask status (2026-07-17)

| Subtask | Status | Implemented/fixed | Still required |
|---|---|---|---|
| P7.1 logging and public telemetry | **Local controls complete; production assurance open** | `4e7d0bb` supplies allow-listed DTO/log minimization; `9822cff` adds the synthetic Railway/Edge log-canary scanner and the full storage/processor retention/access/export/deletion/incident inventory. The audit-identifier decision is now explicit. | Privacy/Security owner must approve actual Railway, Supabase, provider, GitHub, monitoring, support-export, and local-fixture settings; dispose of legacy archives; run and retain deployed canary evidence. |
| P7.2 least-privilege database identity | **Production runtime identity verified; external hardening open** | `4e7d0bb` supplies role/probes and production identity was operator-confirmed; `9822cff` adds a read-only inventory of database/schema/relation/routine/default/PUBLIC privileges and managed consumers. | Run/export the inventory, map non-source consumers, prepare exact replacement/rollback grants, schedule any global change with backup, restrict network where supported, and remove only obsolete application credentials. |
| P7.3 packaging and deployment artifacts | **Local implementation complete; exact production evidence open** | `4e7d0bb` supplies pinned non-root allow-list packaging and exact-SHA evidence; `9822cff` removes `pytest` from runtime dependencies. Local CycloneDX 1.6 generation passed with 22 direct components. | Docker is unavailable locally and Python 3.14 cannot resolve the Python 3.11 pandas/numpy set. Run/review the protected exact-SHA Python 3.11 image, SBOM, full pip-audit, non-root/exclusion, promotion, smoke, and rollback evidence. |
...
| P7.5 dependency/deployment assurance | **Local workflow/runbook complete; production execution open** | `4e7d0bb` supplies exact-SHA workflow/artifacts; `8f4898b` immutably pins the independent frontend release workflow; `9822cff` adds the integrated identity/smoke/rollback evidence record. Local frontend audit is zero-vulnerability; both local SBOM generations pass. | Execute workflows against deployed full SHAs; review findings; record Railway/host/Edge deployment identities, RLS/grants/secrets/replica evidence, named owners, smoke, and rollback rehearsal. |
| P7 exit gate | **Open: local F8 controls complete, operational evidence incomplete** | Backend `9822cff` and frontend `8f4898b` close the remaining identified local assurance-tooling gaps without coupling repositories. | Complete every production-only item in the F8 runbook; do not infer approval or deployed evidence from local implementation. |
...
**F8 frontend release-evidence hardening:** Independent frontend commit `8f4898b` replaces mutable `actions/checkout@v4`, `actions/setup-node@v4`, and `actions/upload-artifact@v4` references in the protected release workflow with immutable official commit SHAs and disables persisted checkout credentials. Six focused release-contract tests, a zero-vulnerability production audit, and a 121-component CycloneDX 1.5 production SBOM passed. Deployment/exact-host-artifact evidence remains manual.
...
- [x] Add a content-free synthetic log-canary scanner and a complete storage/processor decision inventory.
- [ ] Obtain Privacy/Security-owner approval and record the actual vendor retention, access, export, deletion, and incident settings; the operating procedure and decision inventory are defined in the F8 runbook.

**Current state (2026-07-17):** Frontend/Supabase FB.7 minimizes browser/Edge operational logging and frontend `8f4898b` immutably pins its release-evidence actions. Backend `4e7d0bb` provides the privacy boundary; F8 backend `9822cff` adds the content-free deployed-log canary scanner and the Railway/Supabase/provider/GitHub/monitoring/support-export/local-fixture inventory and incident/rollback procedure. The existing privacy policy decides that deleted-user UUIDs remain only in protected append-only audit history for 24 monthly buckets. Actual vendor-console settings, named Privacy/Security approval, legacy archive disposition, and deployed scan evidence remain operational attestations.
...
- [x] Add a read-only inherited/PUBLIC privilege inventory covering managed Supabase consumer roles and default privileges.
...
**Current state (2026-07-17):** Frontend artifact verification remains independent; `8f4898b` pins its release-workflow actions. Backend `4e7d0bb` supplies the packaging/evidence workflow, and `9822cff` removes test-only `pytest` from production requirements. Local CycloneDX 1.6 generation passed with 22 direct components and SHA-256 `3DD2D4EFB14AB874610C04CC05A0CA174E937D105AD0AF0FB227DA8AF7CD6544`. Docker is unavailable locally. The full backend advisory scan remains Unverified because the workstation only has Python 3.14 and cannot resolve the pinned Python 3.11 pandas/numpy build; the protected Python 3.11 workflow is authoritative. Exact artifact promotion and rollback proof remain manual.
...
- The supported topology is attested as exactly one Railway replica with autoscaling disabled and one Uvicorn worker.
- Scaling configuration cannot be enabled accidentally without the shared store.
- If horizontal scaling is later approved, two replicas share actor-bound state/limits consistently and explicit infrastructure failure behavior passes multi-process tests.

**Current backend state (updated 2026-07-17):** Commit `c8bd654` hides process-local rate-limit state behind a repository boundary. F1 rejects configured HTTP worker counts other than one, and F6 commit `4fc5b2d` completes bounded actor/session ownership, expiry, eviction, replay, and turn serialization for the supported one-process topology. Shared state is therefore a future scaling prerequisite, not current mandatory remediation. Exactly one Railway replica with autoscaling disabled remains a manual production control-plane gate.
...
- [x] Generate and review local production SBOM structure for both repositories; backend has 22 direct components and frontend has 121 production components.
...
**Current state (2026-07-17):** Local F8 assurance tooling is complete in backend `9822cff` and independent frontend `8f4898b`, building on the earlier P7 packages and one-replica containment. The phase-wide gate remains open only for production/owner evidence: privacy/vendor approval, legacy archive disposition, deployed log canaries, PUBLIC/network/obsolete-credential maintenance, exact deployed-SHA workflow and artifact identity, one-replica/autoscaling proof, smoke, and rollback rehearsal. Shared state is not required while one replica remains enforced.
...
#### P8.A backend subtask status (2026-07-18)

| Subtask | Status | Implemented/fixed | Still required |
|---|---|---|---|
| P8.1 deep-module extraction | **F9 structural sequence complete** | Existing rate-limit extraction `c8bd654`; startup/readiness `b291018`; actor-bound session runtime `a9cfb57`; provider invocation `65bcb10`; stage orchestration `459f555`; typed plan validation `483b4ce` and compatibility fix `48b65f6`; question-contract finalization `8c36ed8`; evidence derivation `730c0c6`; grounding/provenance gates `5ef774c`. Each change retains a stable compatibility boundary and characterization coverage. | Continue future extraction only when a measured maintenance/risk need justifies it; do not turn this into a big-bang rewrite or combine it with behavior changes. |
...
| P8.2 coverage redistribution | **F9 risk-driven slice complete; ongoing debt remains** | `245aa1b` retained the measured global 80% floor and focused 95% state/database gates. F9 added characterization and failure-path coverage around startup/readiness, session expiry/concurrency, provider invocation, deadline orchestration, planner validation/finalization, evidence derivation, and grounding/provenance. Final suite: `1,669` passed. | Continue adding tests only for newly identified high-risk branches or regressions; do not raise coverage by testing trivial accessors. |
...
| P8.A track completion | **F9 structural work complete; overall P8.A evidence-open** | The requested seven-step F9 extraction order is complete in separate, audited commits, with compatibility fixes separated and the full suite green. | Close the three evidence-dependent decisions above only after representative production evidence and owner approval. |

**Backend implementation evidence (updated 2026-07-18):** Commit `c8bd654` performs the earlier behavior-neutral process-local rate-limit extraction. Commit `245aa1b` makes `pyproject.toml` the sole pytest authority, adds focused state/database-boundary gates, retains the measured global 80% floor, and records the deferred live decisions plus the evidence-based assessment in `p8a_backend_assessment_and_deferred_gates_2026-07-15.md`. F9 then extracted startup/readiness (`b291018`), actor-bound sessions (`a9cfb57`), provider invocation (`65bcb10`), deadline-aware stage orchestration (`459f555`), typed evidence-plan validation (`483b4ce`, with compatibility audit fix `48b65f6`), question-contract finalization (`8c36ed8`), provenance-safe evidence derivation (`730c0c6`), and summary grounding/provenance gates (`5ef774c`). Final F9 local evidence is all `1,669` backend tests passing plus clean Ruff and diff checks. The extraction commits do not require a database migration, frontend/Edge deployment, or new environment variable.
...
- [x] Complete the F9 sequence for startup/readiness, session ownership, provider invocation, pipeline orchestration, planner interpretation/validation, evidence derivation/finalization, and summary grounding/provenance.
- [x] Commit and audit each interface independently; retain compatibility exports where existing tests or extension points require them.
- [x] Re-run risk-driven boundary suites after every extraction and the complete backend suite after the final extraction; final evidence is `1,669` passing tests.
- [x] Preserve public schemas, tool/model-call counts, golden rendering, provenance, and deadline/cancellation behavior throughout F9.
...
- [x] Complete the requested F9 risk-driven characterization slice across `main.py`, provider runtime, orchestration/recovery, session expiry/concurrency, terminal outcomes, and contract/grounding edges.
- [ ] Continue opportunistic risk-driven coverage when a concrete weak branch or regression is identified; do not add trivial accessor tests merely to raise the percentage.
...
**B0 implementation update (2026-07-18):** The append-only release/evidence schema and verified baseline identities are recorded in [`f10_b0_release_evidence_ledger_2026-07-18.md`](./f10_b0_release_evidence_ledger_2026-07-18.md). B0 is closed for repository development after correcting a circular gate: B1/B2 must create candidates before their workflow/deployment/rollback IDs can be recorded. Those IDs, named owners/approvers, protected evidence storage, and green candidate CI are mandatory before B3 production promotion; no High waiver is allowed while waiver authority is unassigned.

**B1 immutable identity update (2026-07-18):** Backend B1.A repository implementation is complete in `dc0620538b949ae6e2879b9b73d7956ddd04d5e2`; focused verification passed `173` tests, the full backend suite passed `1,685` tests, and Ruff is green. Independent frontend/Edge B1.B implementation is complete in `55b15f342796df03dafe29ce77e6c19226cf9fa8`; verification passed `464` frontend tests, ESLint, zero production audit findings, two deterministic production builds, Deno format/lint/type-check, and `21` Edge tests. Browser builds and backend images now bind full SHAs, static smoke verifies every promoted file, and all nine Edge functions report the resolved full version plus source aggregate `973efd2764f9ab31d35789a7cc17edad9ac8dc5c5da9679344cda3fceb2fddcc`. Docker is unavailable locally, so the protected Python 3.11 backend release workflow must perform the real image/non-root/identity verification. This still does not close `F10-REL-02`: exact protected workflow runs, production deployment/image/artifact records, rollback identities, protected archive, named owners, and green candidate CI remain open before B3.
