...
**B1.A backend identity update (2026-07-18):** Repository implementation is complete pending its independent commit and protected deployment evidence. The backend build now binds a required full Git SHA into an OCI revision label and immutable in-image identity; staging/production startup fails without a resolved identity or on an environment/image mismatch; a protected minimal `/versionz` surface supports post-deploy comparison; and the release manifest/workflow cross-check source SHA, image revision, and image ID. Focused verification passed `173` tests, the full backend suite passed `1,685` tests, and Ruff is green. Docker is unavailable locally, so the protected Python 3.11 release workflow must perform the real image/non-root/identity verification. This does not close `F10-REL-02`: B1.B browser/Edge identity, exact production workflow/deployment/image records, rollback identity, protected archive, named owners, and green candidate CI remain open before B3.
