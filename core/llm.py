...
def llm_plan_report_research(
    ...
    *,
    ...
    system = (
        ...
        "KNOWLEDGE_TOPIC_CATALOG lists every topic that approved knowledge "
        "covers. Use it to decide which knowledge tracks are worth planning: a "
        "topic listed there can be retrieved for a track even when "
        "TOPIC_KNOWLEDGE below is empty, which it is whenever the request "
        "names no topic explicitly. Combine closely "
        ...
        "Each track also states how its own evidence should be analysed, "
        "because you wrote its research questions and nothing downstream sees "
        "them. analysis_query_type and analysis_preferred_path classify the "
        "track's leading question the way you would classify a user question; "
        "analysis_answer_kind is the shape of its answer, or null when the "
        "track only supplies documented context. analysis_derived_metrics "
        "names the comparisons the questions actually ask for and is distinct "
        "from requested_metrics: requested_metrics says which quantity to "
        "measure, analysis_derived_metrics says how to compare it over time. "
        "A question asking how a value changed from the previous month is "
        "mom_absolute_change or mom_percent_change; against the same month a "
        "year earlier it is yoy_absolute_change or yoy_percent_change; a "
        "question about a shifting composition is share_delta_mom. "
        "Default to empty. Add a comparison only when a research question "
        "states it in words -- \"how did it change from\", \"compared with\", "
        "\"relative to\" -- and not because a track happens to span two "
        "periods or because a movement would be interesting to know. A "
        "question asking which schedules applied, what something is, or what "
        "a level was gets none. Each entry you add must be traceable to a "
        "specific question you wrote; an entry the evidence cannot satisfy "
        "costs the track every row it collected. "
